...
final class question_xml_writer_test extends \advanced_testcase {
    ...
    /**
     * When a Cloze stem carries bundled media (triggering the HTML media round-trip) and a
     * SHORTANSWER field's answer is literal tag-like text, the field must survive intact:
     * the answer's angle brackets are not parsed as markup and the field is not corrupted.
     *
     * @return void
     */
    public function test_cloze_field_survives_media_reserialisation(): void {
        ...
        $q->type = qti_question::TYPE_CLOZE;
        $q->name = 'Blanks';
        $q->questiontext = '<p>See <img src="pic.png" alt="x"> then {1:SHORTANSWER:=<div>} and {1:SHORTANSWER:=value}.</p>';

        $xml = (new question_xml_writer())->to_moodle_xml([$q], 'cat', $dir);

        // The stem image was rewritten (the media path ran)...
        $this->assertStringContainsString('@@PLUGINFILE@@/pic.png', $xml);
        $dom = new \DOMDocument();
        $this->assertTrue($dom->loadXML($xml), 'output should be well-formed XML');
        $qtext = $dom->getElementsByTagName('questiontext')->item(0)->getElementsByTagName('text')->item(0)->textContent;
        // ...yet the tag-like Cloze field is byte-for-byte intact, not split across markup.
        $this->assertStringContainsString('{1:SHORTANSWER:=<div>}', $qtext);
        $this->assertStringContainsString('{1:SHORTANSWER:=value}', $qtext);
    }
