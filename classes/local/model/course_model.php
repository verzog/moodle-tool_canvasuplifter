...
class course_model {
     ...
     * embed (via $IMS-CC-FILEBASE$ tokens) and so suppressed from the orphan list, keyed
     * by the absolute package path of the file each builds from. The build reconciles
     * these against what activities actually embedded: any whose predicted owner failed
     * to build — so the file was never inlined — is recovered as a standalone download
     * rather than lost. The analyse report ignores this list (it keeps the parse-time
     * prediction), so its output is unchanged. Values are the suppressed {@see item}s.
     */
